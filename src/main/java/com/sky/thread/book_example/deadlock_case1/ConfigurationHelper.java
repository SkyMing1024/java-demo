...
*/
package com.sky.thread.book_example.deadlock_case1;
...
public enum ConfigurationHelper implements ConfigEventListener {
  ...
  public Configuration getConfig(String name) {
    ...
    if (null == cfg) {
      // 持有的是 ConfigurationHelper 当前实例对应的内部锁
      synchronized (this) {
        cfg = getCachedConfig(name);
        if (null == cfg) {
          // 调用了外部方法，此外部方法申请了 ConfigurationManager 当前实例对应的内部锁
  ...
  public void onConfigUpdated(String name, int newVersion,
    ...
    // 申请 ConfigurationHelper 当前实例对应的内部锁
    // 更新内容和版本这两个操作必须是原子操作
    synchronized (this) {
